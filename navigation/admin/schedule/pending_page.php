...
<head>
  ...
  <style>
    #pendingTable th, #pendingTable td { white-space: normal; word-break: break-word; }
  ...
  <ul class="px-3">
    ...
    <li class="p-1 navbar-custom my-2">
      ...
      <ul class="collapse show list-unstyled ps-3 mt-1" id="scheduleSubmenu">
        <li class="my-1"><a class="fw-bold" href="./pending_page.php"><i class="fas fa-hourglass-half me-2"></i>Pending</a></li>
...
<div id="content">
  <div class="container-fluid mt-4">
    ...
    <!-- Removed the extra fallback Print button -->
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-danger" id="modalDeny"><i class="fas fa-times me-1"></i>Deny</button>
        <button type="button" class="btn btn-success" id="modalApprove"><i class="fas fa-check me-1"></i>Approve</button>
      </div>
    </div>
  </div>
</div>
...
$(function () {
  table = $('#pendingTable').DataTable({
    ajax: { url: './fetch_pending_schedules.php?status=Pending', dataSrc: 'data' },
    processing: true,
    autoWidth: false,
    responsive: { details: { type: 'inline' } },
    order: [[0,'desc']],
    // Buttons bar + filter on top; table; info + pagination bottom
    ...
    buttons: [
      {
        ...
        customize: function (win) {
          ...
          header.innerHTML = '<div style="font-size:14px;font-weight:600;margin:0 0 8px;">Pending Schedules</div>';
      ...
      },
      {
        ...
        customize: function (win) {
          const header = win.document.createElement('div');
          header.innerHTML = '<div style="font-size:14px;font-weight:600;margin:0 0 8px;">Pending Schedules — Mass</div>';
          win.document.body.insertBefore(header, win.document.body.firstChild);
          const css = `@page{size:A4 landscape;margin:12mm;}body{font-size:12px!important;}table{width:100%!important;}table.dataTable thead th,table.dataTable tbody td{padding:6px 8px!important;}`;
          const style = win.document.createElement('style');
          style.type = 'text/css';
      ...
      },
      {
        ...
        customize: function (win) {
          const header = win.document.createElement('div');
          header.innerHTML = '<div style="font-size:14px;font-weight:600;margin:0 0 8px;">Pending Schedules — Wedding</div>';
          win.document.body.insertBefore(header, win.document.body.firstChild);
          const css = `@page{size:A4 landscape;margin:12mm;}body{font-size:12px!important;}table{width:100%!important;}table.dataTable thead th,table.dataTable tbody td{padding:6px 8px!important;}`;
          const style = win.document.createElement('style');
          style.type = 'text/css';
    ...
    columnDefs: [
      ...
      { targets: 8, responsivePriority: 1, orderable:false, searchable:false, className: 'no-export' }
    ],
    columns: [
      { data: 'ID' }, // schedules.ID (uppercase)
      { data: null, render: r => `${r.firstname || ''} ${r.lastname || ''}`.trim() },
      { data: null, render: r => `${r.mobilenumber || ''}<br><small>${r.email || ''}</small>` },

      // Other contact person + number
      { data: null, render: r => {
      ...
      { data: null, render: r => `
        <div class="btn-group btn-group-sm" role="group">
          <button class="btn btn-secondary btn-view" data-id="${r.ID}"><i class="fas fa-eye fs-6"></i></button>
          <button class="btn btn-success btn-approve" data-id="${r.ID}"><i class="fas fa-check fs-6"></i></button>
          <button class="btn btn-danger btn-deny" data-id="${r.ID}"><i class="fas fa-times fs-6"></i></button>
        </div>`
      }
    ]
  });
  ...
  // View (modal)
  $('#pendingTable').on('click', '.btn-view', function(){
  ...
  // Approve / Deny from row
  $('#pendingTable').on('click', '.btn-approve', function(){ changeStatus($(this).data('id'), 'Approved'); });
  $('#pendingTable').on('click', '.btn-deny', function(){ changeStatus($(this).data('id'), 'Denied'); });

  // Approve / Deny from modal
  $('#modalApprove').on('click', () => { if(currentRowData) changeStatus(currentRowData.ID, 'Approved'); });
  $('#modalDeny').on('click', () => { if(currentRowData) changeStatus(currentRowData.ID, 'Denied'); });
});
  ...
  fcCal = new FullCalendar.Calendar(container, {
    ...
    events: (fetchInfo, success, failure) => {
      $.getJSON('./fetch_calendar_events.php', { start: fetchInfo.startStr, end: fetchInfo.endStr })
        .done(list => {
          const others = (list || []).map(ev => {
            const isMine = String(ev.userID) === String(CURRENT_USER_ID);
            const classes = [];
            if (isMine) classes.push('event-mine');
            else if ((ev.status || '').toLowerCase() === 'approved') classes.push('event-approved');

            return {
              id: `evt-${ev.id}`,
              title: ev.title || 'Reservation',
              start: ev.start,
              end: ev.end,
              classNames: classes
            };
...
function changeStatus(id, newStatus){
  ...
  Swal.fire({
    ...
    title: `${newStatus} this schedule?`,
    text: isApprove ? 'This will move the reservation to Approved.' : 'This will mark the reservation as Denied.',
    ...
    const $approveBtns = $('.btn-approve, #modalApprove').prop('disabled', true);
    const $denyBtns    = $('.btn-deny, #modalDeny').prop('disabled', true);

    $.post('./update_schedule_status.php', { id, status: newStatus }, function(resp){
      let ok = false, msg = '';
      try{
        const r = typeof resp === 'object' ? resp : JSON.parse(resp);
        ok  = !!r.success;
        msg = r.message || (ok ? `Schedule #${id} marked as ${newStatus}.` : 'Update failed.');
      }catch(e){
        msg = (resp || '').toString().slice(0, 400) || 'Update failed.';
      }

      $approveBtns.prop('disabled', false);
      $denyBtns.prop('disabled', false);

      if(ok){
        const vm = bootstrap.Modal.getInstance(document.getElementById('viewModal'));
        if (vm) vm.hide();

        Swal.fire({ icon:'success', title: isApprove ? 'Approved!' : 'Updated!', text: msg, timer: 1400, showConfirmButton: false })
          .then(()=>{
            if (table) table.ajax.reload(null, false);
            if (fcCal) fcCal.refetchEvents();
          });
      }else{
  ...
  fetch('./auto_update_schedules.php')
    ...
    .then(data => {
      if (data.cancelled > 0 || data.completed > 0) {
        // Silently reload the table to reflect any changes
